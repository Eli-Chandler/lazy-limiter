...
class KeyedTokenBucket:
    ...
    async def acquire(self, key: str, tokens: int | float = 1) -> None:
        ...
        deadline = time.monotonic() + bucket.time_to_capacity(bucket.token_capacity) + self.deletion_grace_period_seconds
    ...
    async def _cleaner(self):
        while True:
            ...
            if bucket.is_full and key in self._token_buckets and self._token_buckets[key] is bucket:
